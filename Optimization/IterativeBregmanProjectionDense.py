...
def WassersteinBarycenterIBP(stride, supps, probs, center_supp, reg, weights=None, fixed_supp=True, numItermax=500,
                             stopThr=1e-4, returnpi=False, verbose=False):
    m = stride.shape[1]
    lp = probs.shape[1]
    d = supps.shape[0]
    ...
    for iter in range(numItermax):
        ...
        if tol < stopThr and not fixed_supp:
            ...
            K[K < 1e-200] = 1e-200
            K = csr_matrix((np.matrix.flatten(K.T), (spIDX_rows, spIDX_cols)), shape=(supp_size * m, lp),
                           dtype=np.float)
            v = np.ones([lp, 1])
            ...
            if f_val > f_val_old:
                if verbose:
                    print('Terminate!', f_val_old, f_val, tol)
                q = q_old
                Pi = Pi_old
