...
def run_clustering():
    ...
    n = 16
    maxiter = 10
    filename = 'BBCdata_d300_n16_tfidf.d2s'
    ...
    label_true = []
    with open("BBCdata_d300_n16_tfidf_labels.d2s", 'r') as f:
        while True:
            la = f.readline()
            if not la:
                break
    ...
    if 1:
        ...
        with open('./results/bbcnews_ami.pkl', 'wb') as f:
            pickle.dump([AMIs], f)

    else:
        with open('./results/bbcsport_ami.pkl', 'rb') as f:
            [AMIs] = pickle.load(f)
    ...
    plt.savefig('figs/exp5_bbcnews.png')
